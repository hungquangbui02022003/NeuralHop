...
class BgeM3Model(XLMRobertaPreTrainedModel):
    ...
    def _process_token_weights(self, token_weights, input_ids, mask):
        ...
        # conver to dict
        all_result = []
        unused_tokens = self.config.unused_tokens or []
    ...
    def forward(
        ...
        if normalize:
            dense_output = torch.nn.functional.normalize(dense_output, dim=-1)
            colbert_output = list(map(torch.nn.functional.normalize, colbert_output))
            sparse_output = list(map(torch.nn.functional.normalize, sparse_output))
